...
pFit, cFit = curve_fit(deuts.deut_real_double_cube_Xi, freqs, newRaw, maxfev=10000,
                       p0=[centfreq,0.021711,0.024969,0.024855,0.036267,0.230673,0.7,0.069351,0.004019,0,0,0,0,0],
                       bounds=([centfreq-0.1,0.0212,0.024,0.0001,0.0001,0.001,0.1,0.05,0,-2,-10000,-10000,-10000,-100000],
                               [centfreq+0.1,0.022,0.03,0.1,0.12,0.5,3,0.25,0.02,2,10000,10000,10000,100000]))
...
print("Reduced chi-squared from data = " + str(deuts.chisq_red(cube_subt,fit,data_std)))
...
fitOx = []
for i in range(nmrwidth):
    resid.append(fit[i]-cube_subt[i])
    onlyOx.append(cube_subt[i] - fitComps[0][i] - fitComps[1][i])
    fitOx.append(fitComps[2][i] + fitComps[3][i])

#This shows a plot of your fit compared to the data.
fig, ax = plt.subplots()
ax.scatter(freqs,cube_subt,label='Data',color='black')
