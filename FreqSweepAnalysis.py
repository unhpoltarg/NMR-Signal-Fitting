...
filename = 'Freq Sweeping\\simp91420.csv'
...
'''pFit, cFit = curve_fit(deuts.deut_double, freqs, flats[0], maxfev=100000,
                           bounds=([32.7,0.0214,0.026,0.01,0,0,1.1,0,0],
                               [33,0.022,0.029,0.04,0.1,0.2,3.0,0.2,0.05]))
temp = []
temp.append(pFit[0])
temp.append(pFit[1])
temp.append(pFit[2])
...
for i in range(nmrwidth):
    ...
    fig, ax = plt.subplots()
ax.scatter(freqs,flats[j],label='Data',color='black')
ax.scatter(freqs,fits[j],label='Fit',color='blue')
plt.legend()
plt.show() '''
...
with open('Freq Sweeping\\920fits.csv', mode='w', newline='', encoding='utf-8-sig') as Sep20File:
    writer = csv.writer(Sep20File, quoting=csv.QUOTE_ALL)
...
with open('Freq Sweeping\\920flats.csv', mode='w', newline='', encoding='utf-8-sig') as Sep20File2:
    writer = csv.writer(Sep20File2, quoting=csv.QUOTE_ALL)
